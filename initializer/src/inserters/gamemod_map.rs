...
pub fn gen_gamemod_map() -> String {
    ...
    gamemod_map.iter().for_each(|ele| {
        sql::gen_value_line(vec![&ele.id_map, &ele.id_gamemod], &mut gamemod_map_value_lines);
    });
    let gamemod_map_inserts = sql::gen_insert("gamemod_map", "(id_map, id_gamemod)", &gamemod_map_value_lines);
